		...
		{	
			...
			if ((logofeat)[j][i].numberOffeat !=0)
			{
				...
				for (k=0 ;k< (logofeat)[j][i].numberOffeat ;k++)
				{
					...
					{
						(double )	CV_MAT_ELEM(*ALLLogoFeatMatrix,double,numberforfeats ,m) =
...
{//pca矩阵 先求模， 再 相乘。
	...
	result=cvCloneMat(result_orignal);	
	pices=result->rows /MAXROWSOF_MATRIX;
	if (result->rows %MAXROWSOF_MATRIX !=0)
	{
		pices++;//最后一个矩阵不满
	}

	resultT=cvCreateMat(result->cols,result->rows,result->type);
	caculateMO(result);//求模
	...
	{//每一个小分的处理
		...
		if (i !=pices -1 )//不是最后一份
		{
			height=MAXROWSOF_MATRIX;
		}
		else
		{
			height=result->rows %MAXROWSOF_MATRIX;
		}
			subMat= cvCreateMatHeader(height, result->cols, result->type);  //创建一个的矩阵头
			rect= cvRect(0,i*MAXROWSOF_MATRIX,subMat->cols,subMat->rows);  //要截取的区域，与创建的矩阵大小一样
			cvGetSubRect(result, subMat, rect);  //pImg为指向图像的指针，pMat指向存储所接图像的矩阵，返回值和pMat相等
			result_result=cvCreateMat(subMat->rows,result->rows,result->type);
		
			cvMatMul(subMat,resultT,result_result);

			mysaveMatrix(result_result,LogoFilepath,i);//保存太大了，不需要double的精度
			cvReleaseMat(&result_result);
	}

	//PrintMatrix(result_orignal);

	
	*numberOfMatrix=pices;

//	PrintMatrix(result);

	//测试用
//	PrintMatrix(*result_result);

	cvReleaseMat(&result);
	cvReleaseMat(&resultT);
}
